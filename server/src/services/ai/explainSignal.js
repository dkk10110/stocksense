...
function templateFallback(detection) {
  ...
  switch (type) {
    ...
    case 'catalyst':
      return `${symbol} has a dated catalyst — <strong>${evidence.catalystLabel}</strong> — in ${evidence.daysToEvent} days, an event that historically moves this kind of stock ~${evidence.expectedImpactPct}%. RSI at ${evidence.rsi} is below the 65 ceiling, so the pre-event drift isn't priced in yet, and ${evidence.fiiDiiAccumulating ? 'FII/DII flow is already accumulating ahead of it' : 'positioning is still light'}. Markets price known events in advance — entering ${evidence.daysToEvent} days out captures the run-up plus the reaction, not the leftovers after the news is public.`;
    default:
